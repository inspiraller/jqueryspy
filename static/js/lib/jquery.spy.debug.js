...
trace('is ajax - append intExpectExtraPrivate');				
...
//trace('timeout intDifference= ' + intDifference);	
//trace('intMaxWaitMS = ' + intMaxWaitMS);					
...
trace('TESTING: strActual = el.' + strJqueryMethod  + '()');
		...
		'ajax.views.each.objectAsValue':function(el, whatExpect, strJqueryMethod){
			...
			//css:{
				...
				//color:green
			//}
trace('ajax.views.each.objectAsValue()');			
trace(arguments);			

trace('strJqueryMethod = ' + strJqueryMethod);	
...
trace('strActual el.'+strJqueryMethod+'(' + expectKey + ') = ');
trace(el);
trace(strActual);
