...
<html>
    <head>
		<title>Jquery Spy &gt; Tutorials &gt; 9</title>		   
...
include(getRoot('/includes/tutorial_nav.php')); ?>   
				...
				<div id="main">
					<h2>Test ajax and multiple elements properties on success</h2>
					<div class="mod">
						<p>Click to view <a href="/tutorials/9/example.htm" target="_blank">Demo</a></p>
<h3>Demo HTML:</h3>						
						<code>
&lt;!DOCTYPE html&gt;
&lt;html&gt;
&lt;head&gt;	   
	...
	&lt;script&gt;
		test("Test ajax and multiple elements properties on success", function () {
			var spy = $.spy({
				ajax:{
					success:{
						views:[{
			...
			$.ajax({
				...
				success:function(){
					$('#qunit-fixture').html('&ltdiv id="someSelector"&gt;Some code here...&lt;/div&gt;');
					$('#someSelector').css({visibility:'visible'});
					$('#someSelector').addClass('george');
				}
			});
			spy.run();		
		});
